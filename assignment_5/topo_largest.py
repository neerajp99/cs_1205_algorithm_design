...
def generateCourseList(course):
    my_course = []
    # print(course)
    for i in range(len(course)):
        my_course.append(course[i])
    #print(my_course)
    return my_course
...
def most_awesome(C, R):
    ...
    # course = list(updatedList)
    # Checking for duplicate values
    # print(Counter(finalAnswer[max(finalArray)][0]).keys())
    # print(Counter(finalAnswer[max(finalArray)][0]).values())

    updatedList = finalAnswer[max(finalArray)][0]
    #course.append(updatedList)
    course = generateCourseList(updatedList)

    print('Maximum Awesomeness is:', max(finalArray))
    print('Maximum Awesomeness Path: ', course)
    print("\n\n")
